...
export const sketch = (p) => {
  ...
  let pg;
  let theShader1;
  let colors = ["#8D0B41", "#D39D55", "#D6CFB4", "#FFF8E6"];
  ...
  p.draw = () => {
    ...
    p.push();
    grid(p, Math.floor(p.random(2, 30)), pg, colors);
    const shaderImage = () => {
      p.shader(theShader1);
...
const base = (p, pg, x, y, w, r) => {
  pg.push();
  pg.translate(x, y);
  for (let i = 0; i < w; i++) {
    let x = i;
    let y = r * p.cos(p.radians(i * 10));
    //pg.circle(x, y, 10);
    pg.line(x, y, x, y + 10);
  }
  pg.pop();
