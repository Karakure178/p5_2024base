...
void main(){
  ...
  // fractでリピートできる
  vec4 tex= texture2D(u_tex, uv+ (celler * 2.));
  //voronoi((uv + vec2(celler * 0.08)), vec2(scale_x, scale_y), vec2(stretch_y, stretch_x), intensity, randomness, 0.);
  //texture2D(u_tex,fract((uv+(celler*0.08))));
