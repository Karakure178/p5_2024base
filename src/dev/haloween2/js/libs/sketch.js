...
export const sketch = (p) => {
  ...
  p.draw = () => {
    ...
    grid(p, pg, num, pgs, colors, rands);
    // gridStroke(p, pg, num, pgs2, rands);
    ...
    // };
    // shaderImage();
    p.image(pg, -pgs[0][0].width / 4, -pgs[0][0].width / 4);
    p.noLoop();
    p.pop();
  };
...
const grid = (p, num, pgs, colors, rands) => {
  for (let i = 0; i < num; i++) {
    for (let j = 0; j < num; j++) {
      pg.push();
      pg.translate(pgs[i][j].width * i, pgs[i][j].height * j);

      pgs[i][j].fill(p.random(colors));
      base(p, pgs[i][j], rands[i][j]);
      pg.image(pgs[i][j], pgs[i][j].width / 2, pgs[i][j].height / 2);
      pg.pop();
    }
  }
};
