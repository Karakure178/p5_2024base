...
export const sketch = (p) => {
  ...
  let pg;
  let theShader1;
  let num = Math.floor(p.random(3, 10));
  let colors = [];
...
const anime = (p, pg, x, y) => {
  pg.push();
  pg.translate(x, y);
  let r = 60 + p.abs(70 * p.pow(p.sin(p.radians(p.frameCount * 10)), 6));
  let num = 6;
  let angle = 360 / num;
  for (let i = 0; i < num; i++) {
    let x = r * p.pow(p.cos(p.radians(i * angle)), 1);
    let y = r * p.pow(p.sin(p.radians(i * angle)), 1);

    let xx = r * p.pow(p.cos(p.radians((i + 1) * angle)), 1);
    let yy = r * p.pow(p.sin(p.radians((i + 1) * angle)), 1);
    pg.push();
    // pg.circle(x, y, 10);
    pg.pop();
...
function base(p, pg, startX, startY, endX, endY, n) {
  for (let i = 0; i <= n; i++) {
    ...
    drawHex(p, pg, x, y, p.random(10, 30), 6);
  }
}
