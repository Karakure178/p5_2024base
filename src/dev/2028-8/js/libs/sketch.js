...
export const sketch = (p) => {
  ...
  let pg;
  let theShader1;
  let colors = ["#690B22", "#E07A5F", "#F1E3D3", "#1B4D3E"];
  ...
  p.draw = () => {
    ...
    // shaderImage();
    grid(p, Math.floor(p.random(2, 100)), pg, colors);
...
const grid = (p, num, pg, colors) => {
  ...
  for (let i = 0; i < num; i++) {
    for (let j = 0; j < num; j++) {
      ...
      drawHex(p, pg, 0, 0, nw / 2, Math.floor(p.random(3, 7)));
      // base(p, pg, nw, nw / 10, colors, 1.0);
      pg.pop();
    }
  }
};

const drawHex = (p, pg, x, y, size, num) => {
  pg.beginShape();
  for (let i = 0; i < num; i++) {
